...
loss_token * loss_token_char(char ch) {
	loss_token *tok = malloc(sizeof *tok);
	tok->n = 1;
	tok->s = strndup(&ch, 1);
	return tok;
}
...
void loss_token_push(loss_token *tok, char ch) {
...
void loss_token_free(loss_token *tok) {
	free(tok->s);
	free(tok);
}
...
loss_token *loss_read_token(FILE *input) {
	int ch;
	...
	loss_token *tok = loss_token_char(ch);
	while (1) {
		int ch = fgetc(input);
		if (ch == EOF) {
			if (!feof(input)) {
